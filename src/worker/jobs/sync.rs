...
pub async fn run(
    ...
    if !report_startup("sync", ready, &readiness, initialize().await) {
        return;
    }
        ...
        tokio::select! {
            ...
            _ = interval.tick() => {
                ...
                match run_once().await {
                    ...
                    Err(error) => tracing::error!(job = "sync", %error, "background job failed"),
                }
            }
        }
    }
...
async fn run_once() -> JobResult {
    // Add periodic synchronization work here.
    Ok(())
}
