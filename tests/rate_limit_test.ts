...
Deno.test("rateLimitGuard — allows requests within limit (per peerAddr)", () => {
  const peer = `10.0.0.${Math.floor(Math.random() * 255)}`;
  for (let i = 0; i < 3; i++) {
    assertEquals(rateLimitGuard(req(), config, peer), null, `Request ${i + 1} should pass`);
  }
});

Deno.test("rateLimitGuard — blocks after exceeding limit (per peerAddr)", () => {
...
Deno.test("rateLimitGuard — different peerAddr have separate limits", () => {
  ...
  assertEquals(rateLimitGuard(req(), config, p2), null, "p2 has its own bucket");
});

Deno.test("rateLimitGuard — prefers CF-Connecting-IP over peerAddr and ignores X-Forwarded-For", () => {
  ...
  assert(blocked !== null, "CF-Connecting-IP identity must survive XFF/peer rotation");
  assertEquals(blocked!.status, 429);
});

Deno.test("rateLimitGuard — does NOT trust X-Forwarded-For / X-Real-IP (spoof rotation can't bypass)", () => {
  ...
  assert(blocked !== null, "rotating spoofable headers must not bypass the per-peer limit");
});
