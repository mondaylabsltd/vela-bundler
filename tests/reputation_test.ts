...
it("ReputationManager - evicts the oldest ok entry at the cap (bounded map)", () => {
  const rep = new ReputationManager({ maxEntries: 3 });
  rep.updateSeen(A, "sender");
  rep.updateSeen(B, "sender");
  rep.updateSeen(C, "sender");
  expect(rep.dump().length).toEqual(3);

  rep.updateSeen(D, "sender"); // at cap → evict oldest ok (A, inserted first)
  const addrs = rep.dump().map((e) => e.address);
  expect(rep.dump().length).toEqual(3);
  expect(!addrs.includes(A.toLowerCase() as `0x${string}`), "A (oldest ok) should have been evicted").toBeTruthy();
  expect(addrs.includes(D.toLowerCase() as `0x${string}`), "D should have been added").toBeTruthy();
});

it("ReputationManager - never evicts a banned entity (penalty is not resettable via flood)", () => {
  const rep = new ReputationManager({ maxEntries: 3 });
  rep.updateSeen(A, "sender");
  rep.updateSeen(B, "sender");
  rep.updateSeen(C, "sender");
  ...
  expect(rep.getStatus(A, "sender")).toEqual("banned");

  rep.updateSeen(D, "sender"); // at cap → must evict an OK entry (B), NOT banned A
  const addrs = rep.dump().map((e) => e.address);
  expect(rep.dump().length).toEqual(3);
  expect(addrs.includes(A.toLowerCase() as `0x${string}`), "banned A must survive eviction").toBeTruthy();
  expect(addrs.includes(D.toLowerCase() as `0x${string}`), "D should have been added").toBeTruthy();
});

it("ReputationManager - stays bounded under a large distinct-sender flood", () => {
  ...
  expect(rep.dump().length <= 100, `map should stay ≤ 100, got ${rep.dump().length}`).toBeTruthy();
});

it("ReputationManager - HARD cap holds even when every entry is penalized (regression)", () => {
  ...
  for (const a of [A, B, C]) {
    ...
    expect(rep.getStatus(a, "sender") !== "ok", "should be penalized").toBeTruthy();
  }
  expect(rep.dump().length).toEqual(3);
  ...
  for (const seed of [D, "0x" + "e5".repeat(20) as `0x${string}`, "0x" + "f6".repeat(20) as `0x${string}`]) {
    ...
    expect(rep.dump().length <= 3, `hard cap breached: ${rep.dump().length}`).toBeTruthy();
  }
});
