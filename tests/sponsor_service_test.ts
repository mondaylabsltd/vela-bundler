...
it("dryRun: eligible probe passes every gate and moves NO money", async () => {
  ...
  try {
    ...
    expect(result).toEqual({ sponsored: false, dryRun: true, eligible: true });
    expect(log.rawTxCount).toEqual(0);
  } finally { restore(); }
});

it("dryRun: repeated probes are served from the probe cache (no extra RPC)", async () => {
  const { log, restore } = stubFetch({ balances: eligibleBalances() });
  try {
    const svc = new SponsorService(config(), fakeAlerter().alerter);
    ...
    expect(second.eligible).toEqual(true);
    expect(log.rpcMethods.length + log.indexQueries).toEqual(callsAfterFirst);
  } finally { restore(); }
});

it("dryRun: an already-funded gas account probes as eligible (needs no grant)", async () => {
  ...
  try {
    ...
    expect(result.reason).toEqual("already_funded");
    expect(log.rawTxCount).toEqual(0);
  } finally { restore(); }
});

it("gate: relayer nonce past the new-user window denies, and the denial is cached", async () => {
  const { log, restore } = stubFetch({ relayerNonce: 7, balances: eligibleBalances() });
  try {
    const svc = new SponsorService(config(), fakeAlerter().alerter);
    const first = await svc.sponsor(1, SAFE_A, RELAYER, RPC);
    expect(first).toEqual({ sponsored: false, reason: "nonce_exceeded" });
...
it("gate: a Safe that cannot cover 2× the sponsor amount is denied (founder policy: no repayment ability → no grant)", async () => {
  const balances = eligibleBalances();
  ...
  try {
    ...
    expect(result).toEqual({ sponsored: false, reason: "wallet_balance_too_low" });
  } finally { restore(); }
});

it("gate: unregistered passkey index entry denies; an index OUTAGE is a distinct retryable reason + alert", async () => {
  const notRegistered = stubFetch({ balances: eligibleBalances(), indexStatus: 404 });
  try {
    const svc = new SponsorService(config(), fakeAlerter().alerter);
    const result = await svc.sponsor(1, SAFE_A, RELAYER, RPC);
    expect(result).toEqual({ sponsored: false, reason: "no_passkey_registered" });
  ...
  try {
    ...
    expect(result).toEqual({ sponsored: false, reason: "passkey_index_unavailable" });
    expect(ids.includes("sponsor-passkey-index-down")).toBeTruthy();
  } finally { outage.restore(); }
});

it("hint cap: an attacker-supplied requiredWei is bounded to 3× the server estimate", async () => {
  ...
  try {
    ...
    expect(result.reason).toEqual("no_passkey_registered");
  } finally { restore(); }
});

it("grant: a fully eligible request transfers once and starts the per-safe success cooldown", async () => {
  const { log, restore } = stubFetch({ balances: eligibleBalances() });
  try {
    const svc = new SponsorService(config(), fakeAlerter().alerter);
    const result = await svc.sponsor(1, SAFE_A, RELAYER, RPC);
    expect(result.sponsored).toEqual(true);
    expect(log.rawTxCount).toEqual(1);
    ...
    expect(again).toEqual({ sponsored: false, reason: "rate_limited" });
    expect(log.rawTxCount).toEqual(1);
  } finally { restore(); }
});

it("budget: the ceiling is hard — the grant after the limit is refused with budget_exhausted + alert", async () => {
  const { log, restore } = stubFetch({ balances: eligibleBalances() });
  try {
    const { alerter, ids } = fakeAlerter();
    const svc = new SponsorService(config(), alerter, { maxGrants: 1 });
    const first = await svc.sponsor(1, SAFE_A, RELAYER, RPC);
    expect(first.sponsored).toEqual(true);
    const second = await svc.sponsor(1, SAFE_B, RELAYER, RPC);
    expect(second).toEqual({ sponsored: false, reason: "budget_exhausted" });
    expect(ids.includes("sponsor-budget-1")).toBeTruthy();
    expect(log.rawTxCount).toEqual(1);
  } finally { restore(); }
});

it("budget: a failed transfer refunds its reservation (the ceiling counts money moved, not attempts)", async () => {
  const failing = stubFetch({ balances: eligibleBalances(), sendRawError: "boom" });
  const { alerter, ids } = fakeAlerter();
  const svc = new SponsorService(config(), alerter, { maxGrants: 1 });
  try {
    const first = await svc.sponsor(1, SAFE_A, RELAYER, RPC);
    expect(first.reason).toEqual("transfer_failed");
    expect(ids.includes("sponsor-transfer-failed-1")).toBeTruthy();
  ...
  try {
    const second = await svc.sponsor(1, SAFE_B, RELAYER, RPC);
    expect(second.sponsored).toEqual(true);
