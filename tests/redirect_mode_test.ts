...
Deno.test("RPC_REDIRECT_MODE - is Cloudflare-compatible ('manual', never 'error')", () => {
  ...
  assertEquals(RPC_REDIRECT_MODE, "manual");
  assert(RPC_REDIRECT_MODE === "follow" || RPC_REDIRECT_MODE === "manual", "must be CF-supported");
  assert((RPC_REDIRECT_MODE as string) !== "error", "redirect:'error' is invalid on Cloudflare Workers");
});
...
Deno.test("reliableTextFetch - does NOT follow a 3xx redirect (SSRF protection preserved)", async () => {
  const clk = fakeClock();
  let calls = 0;
  ...
  const err = await assertRejects(() => reliableTextFetch(
  ...
  assertEquals(getClassification(err).reason, "redirect_blocked");
  // A blocked redirect is not a transient condition → must not be retried.
  assertEquals(calls, 1);
});

Deno.test("reliableTextFetch - opaqueredirect response is also rejected", async () => {
  const clk = fakeClock();
